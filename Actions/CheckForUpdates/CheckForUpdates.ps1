...
$templateFolder = DownloadTemplateRepository -templateUrl $templateUrl -templateSha ([ref]$templateSha) -downloadLatest $downloadLatest -token $token
...
if (-not $isDirectALGo) {
    ...
    if (Test-Path -Path $templateRepoSettingsFile -PathType Leaf) {
        ...
        if ($templateRepoSettings.Keys -contains "templateUrl" -and $templateRepoSettings.templateUrl -ne $templateUrl) {
            ...
            $realTemplateFolder = DownloadTemplateRepository -templateUrl $realTemplateUrl -templateSha ([ref]$realTemplateSha) -downloadLatest ($realTemplateSha -eq '') -token $token
